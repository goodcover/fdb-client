...
import org.apache.spark.sql.proto.ProtobufUtils
import org.apache.spark.sql.proto.ProtobufUtils.{ ProtoMatchedField, toFieldStr }
import org.apache.spark.sql.types.*

/**
 * A serializer to serialize data in catalyst format to data in Protobuf format.
 */
private[sql] class ProtobufSerializer(rootCatalystType: DataType, rootDescriptor: Descriptor, nullable: Boolean) extends Logging {
    ...
    val baseConverter =
      try
        ...
        case ise: AnalysisException =>
          throw QueryCompilationErrors.cannotConvertSqlTypeToProtobufError(rootDescriptor.getName, rootCatalystType, ise)
      }
  ...
  private def newConverter(
    ...
    (catalystType, fieldDescriptor.getJavaType) match {
      ...
      case (StringType, STRING)   =>
        (getter, ordinal) => String.valueOf(getter.getUTF8String(ordinal))
      ...
      case (DayTimeIntervalType(startField, endField), MESSAGE) =>
        (getter, ordinal) =>
          val dayTimeIntervalString =
            IntervalUtils.toDayTimeIntervalString(getter.getLong(ordinal), ANSI_STYLE, startField, endField)
  ...
  private def newStructConverter(
    ...
    val (protoIndices, fieldConverters: Array[Converter]) = protoSchemaHelper.matchedFields.map {
      case ProtoMatchedField(catalystField, _, protoField) =>
        val converter =
          newConverter(catalystField.dataType, protoField, catalystPath :+ catalystField.name, protoPath :+ protoField.getName)
        (protoField, converter)
    }.toArray.unzip

    val numFields = catalystStruct.length

    locally { (row: InternalRow) =>
      val result = DynamicMessage.newBuilder(descriptor)
      var i      = 0
      while (i < numFields) {
        if (row.isNullAt(i)) {
          if (
            !protoIndices(i).isRepeated() &&
