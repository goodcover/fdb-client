...
/**
 * A lockless cache that caches values for multiple users and destroys them once
 * all users release them. One value can be associated with many keys. Useful
 * for sharing a costly resource.
 * @param create
 ...
 */
@nowarn
final class RefCountedCache[K, V](create: K => V, destroy: V => Any, keys: (K, V) => Set[K] = (_: K, _: V) => Set.empty[K]) {

  private[fdb] case class ReleaseTask(value: V, count: Int, scheduledTime: Long) extends Runnable {
    def run(): Unit =
      releaseImmediately(value, count)
  }

  private[fdb] val refCounter       = new RefCountMap[V]
  private[fdb] val cache            = new TrieMap[K, V]
  private[fdb] val valuesToKeys     = new TrieMap[V, Set[K]]
  private[fdb] val deferredReleases = new TrieMap[V, ReleaseTask]
  ...
  /**
   * Acquires a value associated with key. If the value was acquired by another
   * thread and is present in the cache, it will be returned from cache. If the
   * value was not found in cache, a new value will be created by invoking
   * `create` function and will be saved to the cache and associated with `key`
   * and other keys returned by invoking the `keys` function on the value.
   */
  @tailrec
  def acquire(key: K): V =
    cache.get(key) match {
  ...
  private def releaseImmediately(value: V, count: Int = 1): Unit =
    if (refCounter.release(value, count) == 0) {
      // Here we're sure no-one else has the[Any, Unit] value.
  ...
  /**
   * Releases previously acquired value. Once the value is released by all
   * threads and the `releaseDelayMillis` timeout passes, the value is destroyed
   * by calling `destroy` function and removed from the cache.
   */
  ...
  /**
   * Shuts down the background deferred `release` scheduler and forces all
   * pending release tasks to be executed
   */
  def shutdown(): Unit = {
  ...
  /**
   * Removes and destroys unused entries from the cache immediately. Entries
   * with reference count > 0 are not removed.
   */
  def evict(): Unit =
