...
import org.apache.spark.sql.proto.ProtobufUtils.ProtoMatchedField
import org.apache.spark.sql.proto.ProtobufUtils.toFieldStr
  ...
  private val converter: Any => Option[InternalRow] =
    try
      ...
      case ise: AnalysisException =>
        throw QueryCompilationErrors.cannotConvertProtobufTypeToCatalystTypeError(rootDescriptor.getName, rootCatalystType, ise)
    }
  ...
  private def newWriter(
    ...
    (protoType.getJavaType, catalystType) match {
      ...
      case (
            ...
            ArrayType(dataType: DataType, containsNull)
      ...
      case (MESSAGE, StringType) if protoType.getMessageType.getFullName == "google.protobuf.Any" =>
        (updater, ordinal, value) =>
      ...
      case (ENUM, StringType) =>
        (updater, ordinal, value) => updater.set(ordinal, UTF8String.fromString(value.asInstanceOf[EnumValueDescriptor].getName))

      case (ENUM, IntegerType) =>
        (updater, ordinal, value) => updater.setInt(ordinal, value.asInstanceOf[EnumValueDescriptor].getNumber)
  ...
  private def getRecordWriter(
    ...
    val (validFieldIndexes, fieldWriters) = protoSchemaHelper.matchedFields.map {
      case ProtoMatchedField(catalystField, ordinal, protoField) =>
        val baseWriter  =
          newWriter(protoField, catalystField.dataType, protoPath :+ protoField.getName, catalystPath :+ catalystField.name)
