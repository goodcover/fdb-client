 ...
 */
private[sql] class ProtobufOptions(@transient val parameters: CaseInsensitiveMap[String], @transient val conf: Configuration)
    extends FileSourceOptions(parameters)
    with Logging {

  import ProtobufOptions.*
   ...
   * recursive fields are not permitted. Setting it to 0 drops the recursive
   * fields, 1 allows it to be recursed once, and 2 allows it to be recursed
   * twice and so on, up to 10. Values larger than 10 are not allowed in order
   * avoid inadvertently creating very large schemas. If a Protobuf message has
   * depth beyond this limit, the Spark struct returned is truncated after the
   * recursion limit.
   *
   * Examples. Consider a Protobuf with a recursive field: `message Person {
   * string name = 1; Person friend = 2; }` The following lists the schema with
   * different values for this setting. 1: `struct<name: string>` 2:
   * `struct<name string, friend: struct<name: string>>` 3: `struct<name string,
   * friend: struct<name string, friend: struct<name: string>>>` and so on.
   */
   ...
   * 'Any' fields to JSON. At runtime, such 'Any' fields can contain arbitrary
   * Protobuf messages as binary data.
   *
   * By default when this option is not enabled, such field behaves like normal
   * Protobuf message with two fields (`STRUCT<type_url: STRING, value:
   * BINARY>`). The binary `value` field is not interpreted. The binary data
   * might not be convenient in practice to work with.
   *
   * One option is to deserialize it into actual Protobuf message and convert it
   * to Spark STRUCT. But this is not feasible since the schema for
   * `from_protobuf()` is needed at query compile time and can not change at
   * runtime. As a result, this option is not feasible.
   *
   * Another option is parse the binary and deserialize the Protobuf message
   * into JSON string. This this lot more readable than the binary data. This
   * configuration option enables converting Any fields to JSON. The example
   * blow clarifies further.
   *
   * Consider two Protobuf types defined as follows: message ProtoWithAny {
   * string event_name = 1; google.protobuf.Any details = 2; }
   *
   * message Person { string name = 1; int32 id = 2; }
   *
   * With this option enabled, schema for `from_protobuf("col", messageName =
   * "ProtoWithAny")` would be : `STRUCT<event_name: STRING, details: STRING>`.
   * At run time, if `details` field contains `Person` Protobuf message, the
   * returned value looks like this: ('click',
   ...
   *     Any fields should be available in the Protobuf descriptor file passed
   *     to `from_protobuf()`. If any Protobuf is not found, it will result in
   *     error for that record.
   *   - This feature is supported with Java classes as well. But only the
   *     Protobuf types defined in the same `proto` file as the primary Java
   *     class might be visible. E.g. if `ProtoWithAny` and `Person` in above
   *     example are in different proto files, definition for `Person` may not
   *     be found.
   *
   * This feature should be enabled carefully. JSON conversion and processing
   * are inefficient. In addition schema safety is also reduced making
   * downstream processing error prone.
   */
