...
object SchemaConverters extends Logging {
  ...
  def toSqlTypeHelper(descriptor: Descriptor, protobufOptions: ProtobufOptions): SchemaType =
    SchemaType(
      StructType(
        descriptor.getFields.asScala
          .flatMap(structFieldFor(_, Map(descriptor.getFullName -> 1), protobufOptions: ProtobufOptions))
          .toArray
      ),
      nullable = true
    )
  ...
  //// Added Method
  def toSchema(descriptor: Descriptor, protobufOptions: ProtobufOptions): StructType =
    toSqlType(descriptor, protobufOptions).dataType.asInstanceOf[StructType]

  def toSchema(descriptor: Descriptor): StructType =
    toSchema(descriptor, ProtobufOptions(Map.empty))
      ...
      case MESSAGE if fd.isRepeated && fd.getMessageType.getOptions.hasMapEntry =>
        ...
        (keyType, valueType) match {
          ...
          case (_, None)            =>
            ...
            None
          case (Some(kt), Some(vt)) => Some(MapType(kt, vt, valueContainsNull = true))
        }
      case MESSAGE                                                              =>
        ...
        if (
          existingRecordNames.contains(recordName) && (recursiveFieldMaxDepth <= 0 ||
            recursiveFieldMaxDepth > 10)
        ) {
          throw QueryCompilationErrors.foundRecursionInProtobufSchema(fd.toString())
        } else if (
          existingRecordNames.contains(recordName) &&
          ...
          val fields         = fd.getMessageType.getFields.asScala
            .flatMap(
