package org.apache.spark.sql.fdb.metadata
...
import org.apache.spark.sql.fdb.metadata.HDFSMetadataCommitter.META_PATH_SUBDIR
...
/*
   ...
   This was copied from kinesis
 */

class HDFSMetadataCommitter[T <: AnyRef](val path: String, val hadoopConf: SerializableConfiguration)(implicit
  ...
  private val numRetries: Int          = 20     // options.metadataNumRetries
  private val retryIntervalMs: Long    = 100    // options.metadataRetryIntervalsMs
  private val maxRetryIntervalMs: Long = 10_000 // options.metadataMaxRetryIntervalMs
...
//    val reader = new InputStreamReader(in, StandardCharsets.UTF_8)
  ...
  def add(batchId: Long, shardId: String, metadata: T): Boolean = {
    ...
    val output = fileContext.create(
      shardCommitPath,
      util.EnumSet.of(CREATE, OVERWRITE),
      ...
      try
        ...
        case NonFatal(e) => // re-throw the exception with the log file path added
          throw new IllegalStateException(
            s"Failed to read log file $path. " +
              s"${e.getMessage}",
            e
          )
      } finally
  ...
  def withRetry[U](message: String, ignoreException: Boolean = true)(body: => U): U = {
    ...
    result.getOrElse {
      throw new IllegalStateException(
        s"Gave up after $retryCount retries while $message," +
          s" last exception: ",
        lastError
      )
    }
