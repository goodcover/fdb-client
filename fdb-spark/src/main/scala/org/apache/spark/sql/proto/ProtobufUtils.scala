...
private[sql] object ProtobufUtils extends Logging {
   ...
   * Protobuf field.
   */
  private[sql] case class ProtoMatchedField(catalystField: StructField, catalystPosition: Int, fieldDescriptor: FieldDescriptor)

  /**
   * Helper class to perform field lookup/matching on Protobuf schemas.
   *
   * This will match `descriptor` against `catalystSchema`, attempting to find a
   * matching field in the Protobuf descriptor for each field in the Catalyst
   * schema and vice-versa, respecting settings for case sensitivity. The match
   ...
   */
  class ProtoSchemaHelper(descriptor: Descriptor, catalystSchema: StructType, protoPath: Seq[String], catalystPath: Seq[String]) {
    if (descriptor.getName == null) {
      throw QueryCompilationErrors.unknownProtobufMessageTypeError(descriptor.getName, descriptor.getContainingType().getName)
    }
    ...
    /**
     * The fields which have matching equivalents in both Protobuf and Catalyst
     * schemas.
     */
    val matchedFields: Seq[ProtoMatchedField] = catalystSchema.zipWithIndex.flatMap { case (sqlField, sqlPos) =>
     ...
     * Protobuf field, throwing [[AnalysisException]] if such extra fields are
     * found. If `ignoreNullable` is false, consider nullable Catalyst fields to
     * be eligible to be an extra field; otherwise, ignore nullable Catalyst
     * fields when checking for extras.
     */
    def validateNoExtraCatalystFields(ignoreNullable: Boolean): Unit =
      catalystSchema.fields.foreach { sqlField =>
        if (
          getFieldByName(sqlField.name).isEmpty &&
          (!ignoreNullable || !sqlField.nullable)
        ) {
          throw QueryCompilationErrors.cannotFindCatalystTypeInProtobufSchemaError(toFieldStr(catalystPath :+ sqlField.name))
        }
      }

    /**
     * Validate that there are no Protobuf fields which don't have a matching
     * Catalyst field, throwing [[AnalysisException]] if such extra fields are
     * found. Only required (non-nullable) fields are checked; nullable fields
     * are ignored.
     */
    def validateNoExtraRequiredProtoFields(): Unit = {
      ...
      extraFields.filter(_.isRequired).foreach { extraField =>
        throw QueryCompilationErrors.cannotFindProtobufFieldInCatalystError(toFieldStr(protoPath :+ extraField.getName()))
      }
    }

    /**
     * Extract a single field from the contained Protobuf schema which has the
   ...
   * serialized descriptor read from the file.
   * @param messageName
   *   Protobuf message name or Java class name (when binaryFileDescriptorSet is
   *   None)..
   * @param binaryFileDescriptorSet
   *   When the binary `FileDescriptorSet` is provided, the descriptor and its
   *   dependencies are read from it.
   * @return
   */
    ...
    val getDescriptorMethod =
      try
        ...
        case e: NoSuchMethodError => // This is usually not expected.
          throw QueryCompilationErrors.protobufClassLoadError(protobufClassName, "Could not find getDescriptor() method", e)
      }
  ...
  def buildDescriptor(binaryFileDescriptorSet: Array[Byte], messageName: String): Descriptor = {
    ...
    val descriptorOpt = parseFileDescriptorSet(binaryFileDescriptorSet).flatMap { fileDesc =>
  ...
  private def parseFileDescriptorSet(bytes: Array[Byte]): List[Descriptors.FileDescriptor] = {
    ...
    val fileDescriptorList: List[Descriptors.FileDescriptor]  =
      fileDescriptorSet.getFileList.asScala
        .map(fileDescriptorProto => buildFileDescriptor(fileDescriptorProto, fileDescriptorProtoIndex))
        .toList
    fileDescriptorList
  }
    ...
    val fileDescriptorList = fileDescriptorProto.getDependencyList().asScala.map { dependency =>
      fileDescriptorProtoMap.get(dependency) match {
        case Some(dependencyProto) =>
          if (
            dependencyProto.getName == "google/protobuf/any.proto"
  ...
  /**
   * Returns a map from descriptor proto name as found inside the descriptors to
   * protos.
   */
   ...
   */
  private[proto] def buildTypeRegistry(descriptorBytes: Array[Byte]): TypeRegistry = {
   ...
   * proto file.
   */
  private[proto] def buildTypeRegistry(descriptor: Descriptor): TypeRegistry =
    TypeRegistry
      .newBuilder()
