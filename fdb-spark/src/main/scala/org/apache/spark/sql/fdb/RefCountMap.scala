...
class RefCountMap[T] {
  ...
  /**
   * Returns current reference count for the given key. This value may be
   * constantly changing, so do not use it for synchronization purposes.
   */
  final def get(key: T): Int =
    refCounts.getOrElse(key, 0)

  /**
   * Atomically increases reference count only if the reference counter is
